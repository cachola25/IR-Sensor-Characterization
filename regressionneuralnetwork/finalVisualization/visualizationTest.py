...
TEST_MODE = True  # Set to False when near the Roomba
...
async def play(robot):
    ...
    while True:
        try:
            ...
            predicted_distance = max(0, prediction[0] * 550)  # Clamp to 0+, scale
            predicted_start_deg = np.degrees(prediction[1]) % 180
            predicted_end_deg = np.degrees(prediction[2]) % 180
...
async def fake_prediction_loop():
    ...
    while True:
        ...
        input_data = np.array(ir_values).reshape(1, -1) / MAX_SENSOR_VALUE
        prediction = model.predict(input_data, verbose=0)[0]
        predicted_distance = max(0, prediction[0] * 550)
        predicted_start_deg = np.degrees(prediction[1]) % 180
        predicted_end_deg = np.degrees(prediction[2]) % 180
...
def draw_arc_with_grid(surface, center, radius, start_angle, end_angle, sector_angle):
    ...
    for angle in range(start_angle, end_angle + 1, sector_angle):
        ...
        arc_surface.blit(label, (end_x - 10, end_y))
    ...
    for i in range(1, 7):
        ...
        # Start labeling from the 2nd ring as "2in"
        if i >= 2:
            inches = (i - 1) * 2  # i=2 → 2in, i=3 → 4in, ..., i=6 → 10in
...
def draw_prediction_cone(surface, center, radius, start_angle, end_angle, distance):
    arc_surface = pygame.Surface((radius * 2, radius * 2), pygame.SRCALPHA)
    arc_surface.fill((0, 0, 0, 0))
    distance = max(5, min(distance, radius))
...
async def main():
    if not TEST_MODE:
        ...
        print("✅ Roomba play triggered!")
    else:
        print("🧪 TEST MODE: Running with fake sensor data!")

    tasks = [run_game()]
    if TEST_MODE:
        tasks.append(fake_prediction_loop())
    else:
